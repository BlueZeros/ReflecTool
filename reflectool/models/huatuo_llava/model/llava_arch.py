...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal_new(
        ...
        if not self.training: # TODO: check this out!!
            # isinstance(input_ids, torch.Tensor) ensures that we won't enter this when doing evaluation at training
            if vision_tower is None or images is None or (isinstance(input_ids, torch.Tensor) and input_ids.shape[1] == 1):
                if past_key_values is not None and vision_tower is not None and images is not None and input_ids.shape[1] == 1:
                    target_shape = past_key_values[-1][-1].shape[-2] + 1
                    attention_mask = torch.cat((attention_mask, torch.ones(
                        (attention_mask.shape[0], target_shape - attention_mask.shape[1]),
        ...
        if (len(images) == 0):
                ...
                for image_idx, image_feature in enumerate(image_features):
                    if image_feature.shape[0] > 1:
                        ...
                        if 'anyres' in image_aspect_ratio:
                            ...
                            # 默认都是选第一个
                            big_width, big_height = self.config.image_grid_pinpoints[0]
                            num_patch_width, num_patch_height = big_width // patch_size, big_height // patch_size
            ...
            # print(f'trained image shape {images.shape}')
            image_features = self.encode_images(images).to(self.device)
        ...
        if labels is None:
            labels = [torch.tensor([IGNORE_INDEX]*l).to(input_ids[0]) for l in map(len, [ip for ip in input_ids])]
            # labels = torch.full_like(input_ids, IGNORE_INDEX)
        else:
            labels = [cur_labels[cur_attention_mask] for cur_labels, cur_attention_mask in zip(labels, attention_mask)]
        ...
        # assert images are not offser
        assert cur_image_idx == len(image_features), f'wrong!! cur_image_idx != len(image_features) cur_image_idx:{cur_image_idx} len(image_features):{len(image_features)}'
        ...
        # print(new_input_embeds.shape) # should be
