...
conv_meditron = Conversation(
    system="""You are a helpful, respectful, and honest assistant. Always answer as helpfully as possible while being safe. Your answers should not include any harmful, unethical, arcist, sexist, toxic, dangerous, or illegal content. Please ensure that your responses are socially unbiased and positive in nature.\n\nIf a question does not make any sense, or is not factually coherent, explain why instead of answering something not correct. If you don't know the answer to a question, please don't share false information.""",
    roles=("User", "Assistant"),
    messages=(),
    offset=0,
    sep_style=SeparatorStyle.SINGLE,
...
conv_templates = {
    ...
    "mpt": conv_mpt,
    "qwen": conv_qwen,
    "meditron": conv_meditron,
    "llavamedpp": conv_llavamedpp,
}

def get_conv(model_name):
    if model_name in conv_templates:
        return conv_templates[model_name]
    else:
        for conv_name in conv_templates:
            if conv_name in model_name:
