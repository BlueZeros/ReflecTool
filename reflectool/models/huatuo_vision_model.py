...
# from models.HuatuoGPT_Vision.cli import HuatuoChatbot

import sys
from reflectool.models.huatuo_llava.constants import IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from reflectool.models.huatuo_llava.model import *
...
class HuatuoChatbot():
    ...
    def init_components(self):
        ...
        if 'huatuogpt-vision-7b' in d.lower():
            ...
            from models.huatuo_llava.model.language_model.llava_qwen2 import LlavaQwen2ForCausalLM
            ...
            print(f'loading from {self.model_dir}')
            from  models.huatuo_llava.model.language_model.llava_llama import LlavaLlamaForCausalLM
...
class HuatuoVision_Model(Base_Model):
    def __init__(self, model_name, system_prompt, stops, cuda_id=0):
        super().__init__()

        self.model = HuatuoChatbot(LOCAL_MODEL_PATHS[model_name], device=f'cuda:{cuda_id}') # loads the model 
    
    def __call__(self, inputs: str, images: str = None):
        # if images is None:
        #     matches = re.findall(r'Inputs:\s*(/[^ \n]+)', inputs)
        #     if len(matches) != 0:
        #         image_path = matches[0]
        #         if os.path.exists(image_path):
        #             images = image_path
        #             inputs.replace(image_path, "")

        return self.model.inference(inputs, images)[0]