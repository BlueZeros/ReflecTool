...
class ReflexionAgent(TaskAgent):
    ...
    def reflexion(self, task):
        action_chain = self.short_term_memory.get_action_chain(task)
        ltm = self.long_term_memory.get_memories(task)

        reflextion_prompt = self.prompt_gen.reflexion_prompt(
            task=task,
            action_chain=action_chain,
            examples=ltm,
        )
        self.logger.get_prompt(reflextion_prompt)

        reflextion = self.llm_layer(reflextion_prompt)
