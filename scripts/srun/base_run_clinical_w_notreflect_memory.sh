...
EXP_NAME=clinical_${SEARCH}-${MODEL}-few_shot_${FEWSHOT}-${ACTIONS}-${MEMORY_TYPE}_memory_wo_reflect
...
srun --jobid $SLURM_JOBID python run.py \
    ...
    --model ${MODEL} \
    --actions ${ACTIONS} \
    --action-guide-path "../results/train/${TASK_SPLIT}/train-qwen2-72b-int4-few_shot_0-all/action_optim/step-200.json" \
    --memory-path ${MEMORY} \
