...
#         sbatch -J eval_${domain} --gres=gpu:2 -x SH-IDC1-10-140-1-159,SH-IDC1-10-140-0-228 ../scripts/srun/base_run_clinical_w_mm_memory_reflect_num.sh $DATA_PATH $OUTPUT_PATH $TASK_NAME $domain $MODEL $ACTIONS $MEMORY $MEMORY_TYPE $FEWSHOT ${SEARCH} ${REFLECT_NUM} & sleep 3
#     done
# done
...
for domain in "${domains[@]}"; do
    MEMORY=${DATA_PATH}/memory
    for REFLECT_NUM in "${REFLECT_NUMS[@]}"; do
        sbatch -J eval_${domain} --gres=gpu:2 -x SH-IDC1-10-140-1-159 ../scripts/srun/base_run_clinical_w_mm_memory_reflect_num.sh $DATA_PATH $OUTPUT_PATH $TASK_NAME $domain $MODEL $ACTIONS $MEMORY $MEMORY_TYPE $FEWSHOT ${SEARCH} ${REFLECT_NUM} & sleep 3
    done
done
