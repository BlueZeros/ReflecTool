...
for domain in "${domains[@]}"; do
    ...
    for WARM_UP_SIZE in "${WARM_UP_SIZES[@]}"; do
        sbatch -J eval_${domain} -x SH-IDC1-10-140-1-159 ../scripts/srun/base_run_clinical_w_memory_warmupsize.sh $DATA_PATH $OUTPUT_PATH $TASK_NAME $domain $MODEL $ACTIONS $MEMORY $MEMORY_TYPE $FEWSHOT ${SEARCH} ${WARM_UP_SIZE} & sleep 3
    done
done
