...
for domain in "${domains[@]}"; do
    sbatch -J train_${domain} --gres=gpu:2 -x SH-IDC1-10-140-0-229  ../scripts/srun/base_run_train_w_memory.sh $DATA_PATH $OUTPUT_PATH $TASK_PATH $domain $MODEL $ACTIONS $MEMORY $FEWSHOT & sleep 3
done
